<script lang="ts">
  import { isFileType, isMaxFileSizeMb } from "@/lib/utils";
...
<div
  class="file-drop relative flex max-w-sm select-none items-center justify-center rounded-lg border-2 border-dashed border-neutral-100 border-opacity-50 bg-opacity-25 p-8 pb-4 transition-colors"
  class:border-opacity-100={isDragOver}
  class:bg-neutral-500={isDragOver}
  class:disabled
>
  <div
    class="relative z-10 opacity-100 transition-opacity"
    class:opacity-20={isDragOver}
    class:!z-0={isDragOver}
  >
    <label
      class="block rounded-full border bg-opacity-30 px-8 py-2 text-center shadow-sm transition-colors hover:cursor-pointer hover:bg-opacity-50"
    ...
    >
      ...
      <input
        ...
        {accept}
        {multiple}
        class="absolute -z-30 block -translate-x-full opacity-0"
      />
    </label>
