...
describe("Hyper5HeadMode", () => {
  ...
  it("should render correctly", async () => {
    ...
    expect(screen.getByLabelText("Custom Command")).toBeInTheDocument();
  ...
  it("enable custom command input when custom command is selected", async () => {
    ...
    expect(screen.getByLabelText("Custom Command")).not.toBeChecked();
    await user.click(screen.getByLabelText("Custom Command"));
